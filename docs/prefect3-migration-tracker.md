...
| acled | deploy + smoke | **rebuilt** from manual QGIS rasterization; now filter_and_agg (GPKG); manual CSV download (ACLED requires registration); `event_count` + `fatalities` outcomes; backend processor `acled_dynamic_filter_and_agg` added; moved from Workstream B |
| ucdp | deploy + smoke | **rebuilt**, consolidating what were 2 top-level scripts + an abandoned per-violence-type WIP (`type_and_year/`) into one flow: one GED CSV download produces 4 fatality-sum outputs (all/state-based/non-state/one-sided). Dropped the GED Polygons-based binary conflict-occurrence output — that dataset (v1.1, 2012) is no longer listed on UCDP's downloads page, confirmed gone. Caught a real bug in the abandoned WIP script: it rasterized points directly with `attribute=`, which keeps only one event's value per pixel rather than summing overlapping events (verified against real data: ~38% undercount in a sample year/category); the new flow uses the original top-level script's correct round-to-grid-then-groupby-sum approach instead. Moved from Workstream B |
| gcdf_v3_dynamic | deploy + smoke | **rebuilt** from a standalone script (`type: feature`/`filter_and_agg`, not raster — a different shape than most of this tracker). Downloads AidData's GCDF v3.0.1 buffered-project-polygon GeoPackage from GitHub releases, renames/subsets to the filter fields, and refreshes `filter_ingest.json`'s year ranges/category lists from the actual data on every run. `gcdf_v3_boundaries`/`gcdf_v3_static` are out of scope — generated by a separate repo (`aiddata/china-osm-geodata`), not a flow here. Full pipeline (download, rename/subset, GPKG round-trip, filter values) verified against the real ~496MB release before committing — filter values matched the already-checked-in `filter_ingest.json` exactly |
...
| modis_landcover | deploy + smoke | **rebuilt** from GLCF v5.1 FTP shell scripts; now LP DAAC LPCLOUD (MCD12Q1 v061); CMR search → HDF4 download → `gdal_translate` tile extraction → `gdal_merge.py`+`gdalwarp` mosaic/reproject to WGS84; Earthdata bearer token; moved from Workstream B |
...
### Earthdata bearer token — oco2, ltdr_ndvi, distance_to_groads, modis_landcover

All four pull from NASA Earthdata Login services (oco2 = GES DISC, ltdr =
LAADS, distance_to_groads = SEDAC, modis_landcover = LP DAAC LPCLOUD); a
single token from urs.earthdata.nasa.gov authenticates CMR granule search and
downloads across all of them via `Authorization: Bearer <token>`.
Field/`.env` key/`__main__` lookup all named `earthdata_token`. Confirmed
live: an anonymous request to the SEDAC download URL redirects through
`urs.earthdata.nasa.gov/oauth/authorize` to a 401 Basic-auth challenge; the
same bearer token already used for ltdr_ndvi/oco2 works here too
(303 → signed CloudFront/S3 URL → real zip bytes).
...
`gcdf_v3` (partial — see note below; `gcdf_v3_boundaries`/`gcdf_v3_static` are
generated by a separate repo, out of scope here)

`globalsolaratlas` — has `pvout_raster_ingest.json` + README but no flow or
config.toml; not yet tracked. Triage before investing.

`globalwindatlas` — has `pd_raster_ingest.json`, `ws_raster_ingest.json` + README
but no flow or config.toml; not yet tracked. Triage before investing.

`srtm` — abandoned; replaced by `gebco2026` (SRTMGL3 has 14,297 tiles, not
tractable to mosaic). No flow needed.
...
- **modis_landcover rebuilt** (commit `b184eb1`): moved from Workstream B into A. Was a GLCF v5.1 FTP `wget` + `gdal_merge.py` shell script pair; now a Dataset/flow against LP DAAC LPCLOUD (MCD12Q1 v061, CMR concept C2484079608-LPCLOUD). Pipeline: CMR search paginates ~317 HDF4 tile granules per year → Bearer-token download → `gdal_translate` extracts `LC_Type1` EOS grid subdataset from each tile → `gdal_merge.py` mosaics in MODIS sinusoidal → `gdalwarp -r near` reprojects to WGS84. IGBP class mappings unchanged from v5.1. Same Earthdata bearer token pattern as oco2/ltdr_ndvi/distance_to_groads.

- **acled rebuilt** (commit `14ccc16`): moved from Workstream B into A. Was an unmigrated raster produced manually in QGIS; now a Dataset/flow expecting a manually downloaded ACLED CSV in `raw_dir`, converts to a GeoPackage, and refreshes `acled_filter_ingest.json` with live year ranges, event-type categories, and fatality ranges. Processing type: `filter_and_agg` (same shape as `ucdp`). Backend processor `acled_dynamic_filter_and_agg` added to geoquery-update. Outcomes: `event_count` (synthetic, 1/event) and `fatalities`. Old `raster_ingest.json` retained until the new ingest is live.

- **geoboundaries + TIGER READMEs added** (this commit): both were in the Done section with `main.py`/`config.toml` but no README. Added dataset-specific READMEs for both. Confirmed neither uses a committed ingest JSON (geoboundaries = boundary download utility; TIGER = generates ingest JSON dynamically per run).

- **globalsolaratlas + globalwindatlas noted in Workstream B**: both directories have ingest JSONs and READMEs but no flow or config.toml. Added to tracker for triage.
